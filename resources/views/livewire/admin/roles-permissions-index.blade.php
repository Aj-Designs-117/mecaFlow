                            ...
                            <td class="px-4 py-2 flex justify-center gap-2">
                                @can('admin.roles-permissions.permissions')
                                    <flux:modal.trigger name="assig-permissions">
                                        <flux:button wire:click="assingPermissions('{{ $role->id }}')" variant="primary" class="cursor-pointer hover:bg-yellow-200" size="sm">🗝️</flux:button>
                                    </flux:modal.trigger>
                                @endcan
    ...
    <flux:modal name="assig-permissions" class="md:w-96">
        <form wire:submit.prevent="update('{{ $id }}')">
            <div class="space-y-6">
                ...
                <flux:input wire:model="name" :label="__('Name')" required autofocus autocomplete="Nombre" placeholder="Ingrese el nombre" />

                <flux:checkbox.group label="Permisos">
                    @foreach($permissions as $permission)
                        <flux:checkbox wire:model="selectedPermissions" label="{{ $permission->description }}" value="{{ $permission->name }}" />
                    @endforeach
                </flux:checkbox.group>
