                                    ...
                                    <td class="px-4 py-2 flex justify-center gap-2">
                                        @can('admin.categories.edit')
                                            <flux:modal.trigger name="editCategory">
                                                <flux:button wire:click="edit('{{ $category->id }}')" variant="primary"
                                                    class="cursor-pointer hover:bg-sky-200" size="sm">✏️</flux:button>
                                            </flux:modal.trigger>
                                        @endcan
                                        @can('admin.categories.destroy')
                                            <flux:button wire:click="destroy('{{ $category->id }}')" variant="primary"
    ...
    <flux:modal name="editCategory" class="md:w-96">
        <form wire:submit.prevent="update('{{ $id }}')">
            <div class="space-y-6">
                ...
                <flux:input wire:model="slug" :label="__('Slug')" type="text" required autofocus
                    autocomplete="slug" placeholder="Ingrese el slug" />
