    ...
    <flux:sidebar sticky stashable class="border-r border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Platform')" class="grid">
                ...
                <flux:navlist.item icon="chart-bar-square" :href="route('dashboard')"
                ...
                @can('admin.posts.index')
                    <flux:navlist.item icon="clipboard-document-list" :href="route('admin.posts.index')"
                        :current="request() - > routeIs('admin.posts.index')" wire:navigate>{{ __('Lista de Posts') }}
                    </flux:navlist.item>
                @endcan
                @can('admin.posts.index')
                    <flux:navlist.item icon="plus-circle" :href="route('admin.posts.create')"
                        :current="request() - > routeIs('admin.posts.create')" wire:navigate>{{ __('Crear posts') }}
                    </flux:navlist.item>
                @endcan
                @can('admin.categories.index')
                    <flux:navlist.item icon="tag" :href="route('admin.categories.index')"
                        :current="request() - > routeIs('admin.categories.index')" wire:navigate>{{ __('Categorias') }}
                    </flux:navlist.item>
                @endcan
            </flux:navlist.group>
                ...
                <flux:navlist.group :heading="__('Jerarquia')" class="grid">
                    @can('admin.users.index')
                        <flux:navlist.item icon="users" :href="route('admin.users.index')"
                            :current="request() - > routeIs('admin.users.index')" wire:navigate>{{ __('Usuarios') }}
                        </flux:navlist.item>
                    @endcan
                    @can('admin.roles-permissions.index')
                        <flux:navlist.item icon="tag" :href="route('admin.rolesPermissions.index')"
                            :current="request() - > routeIs('admin.rolesPermissions.index')" wire:navigate>
                            {{ __('Roles & Permisos') }}</flux:navlist.item>
                    @endcan
                    @can('admin.roles.index')
                        <flux:navlist.item icon="user" :href="route('admin.roles.index')"
                    ...
                    @can('admin.permissions.index')
                        <flux:navlist.item icon="key" :href="route('admin.permissions.index')"
                            :current="request() - > routeIs('admin.permissions.index')" wire:navigate>{{ __('Permisos') }}
                        </flux:navlist.item>
                    @endcan
                </flux:navlist.group>
            @endcan
